/**
 * jQuery CoreUISelect
 * @version     Modified for jQuery 1.9.1 by Ivan Ivanov
 */
(function ($) {
    ...
    CoreUISelect.prototype.hideDomSelect = function() {
        ...
        if(browser.mobile){ this.domSelect.css({'z-index' : 1})};
    };
    ...
    CoreUISelect.prototype.bindUIEvents = function() {
        ...
        //if(browser.mobile){this.select.on(customClickEvent, function() { _that.domSelect.trigger('touchstart');})};
        if(browser.mobile){this.domSelect.on('change', $.proxy(this, 'changeDropdownData'))};
        this.select.on(customClickEvent, $.proxy(this, 'onSelectClick'));
        this.dropdownItem.on(customClickEvent, $.proxy(this, 'onDropdownItemClick'));
    }
    ...
    CoreUISelect.prototype.destroy = function() {
        ...
        this.domSelect.off('change', $.proxy(this, 'onChange'));
        this.select.off(customClickEvent, $.proxy(this, 'onSelectClick'));
        this.dropdownItem.off(customClickEvent, $.proxy(this, 'onDropdownItemClick'));
    ...
    $(document).on('keyup', function(event){
        for(var i=0; i<allSelects.length; i++){
            if(browser.safari || browser.msie || browser.opera || browser.mozilla) allSelects[i].changeDropdownData(event); // Hack for Safari
