 ...
 * Last Modified: 2013-02-11T16:35:46+01:00
 * File:          app/code/local/Xtento/ProductExport/Model/Destination/Local.php
 * Copyright:     Copyright (c) 2014 XTENTO GmbH & Co. KG <info@xtento.com> / All rights reserved.
 */

class Xtento_ProductExport_Model_Destination_Local extends Xtento_ProductExport_Model_Destination_Abstract
{
    public function testConnection()
    {
        ...
        foreach ($forbiddenFolders as $forbiddenFolder) {
            if (@realpath($exportDirectory) == $forbiddenFolder) {
                return $testResult->setSuccess(false)->setMessage(Mage::helper('xtento_productexport')->__('It is not allowed to save export files in the directory you have specified. Please change the local export directory to be located in the ./var/ folder for example. Do not use the Magento root directory for example.'));
            }
        }
        ...
        if (!is_dir($exportDirectory)) {
            ...
            if (!@mkdir($exportDirectory)) {
                return $testResult->setSuccess(false)->setMessage(Mage::helper('xtento_productexport')->__('The specified local directory does not exist. We could not create it either. Please make sure the parent directory is writable or create the directory manually: %s', $exportDirectory));
            } else {
        ...
        if (!$this->_connection || @!is_writable($exportDirectory)) {
            return $testResult->setSuccess(false)->setMessage(Mage::helper('xtento_productexport')->__('Could not open local export directory for writing. Please make sure that we have rights to read and write in the directory: %s', $exportDirectory));
        }
        if ($testResult->getDirectoryCreated()) {
            $testResult->setSuccess(true)->setMessage(Mage::helper('xtento_productexport')->__('Local directory didn\'t exist and was created successfully. Connection tested successfully.'));
            ...
            $testResult->setSuccess(true)->setMessage(Mage::helper('xtento_productexport')->__('Local directory exists and is writable. Connection tested successfully.'));
            if (!$this->getDestination()->getBackupDestination()) {
                $this->getDestination()->setLastResult($testResult->getSuccess())->setLastResultMessage($testResult->getMessage())->save();
            }
            return $testResult;
    ...
    public function saveFiles($fileArray)
    {
        ...
        if (!$testResult->getSuccess()) {
            ...
            $logEntry->addResultMessage(Mage::helper('xtento_productexport')->__('Destination "%s" (ID: %s): %s', $this->getDestination()->getName(), $this->getDestination()->getId(), $testResult->getMessage()));
            if (!$this->getDestination()->getBackupDestination()) {
