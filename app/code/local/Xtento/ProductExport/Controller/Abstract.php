 ...
 * Last Modified: 2013-11-09T15:49:48+01:00
 * File:          app/code/local/Xtento/ProductExport/Controller/Abstract.php
 * Copyright:     Copyright (c) 2014 XTENTO GmbH & Co. KG <info@xtento.com> / All rights reserved.
 */

class Xtento_ProductExport_Controller_Abstract extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    protected function _prepareFileDownload($fileArray)
    {
        if (count($fileArray) > 1) {
            ...
            if (!@class_exists('ZipArchive') && !function_exists('gzopen')) {
                $this->_getSession()->addError(Mage::helper('xtento_productexport')->__('PHP ZIP extension not found. Please download files manually from the server, or install the ZIP extension, or export just one file with each profile.'));
            ...
            if (@class_exists('ZipArchive')) {
                ...
                if ($zipArchive->open($zipFile, ZIPARCHIVE::CREATE) !== TRUE) {
                    $this->_getSession()->addError(Mage::helper('xtento_productexport')->__('Could not open file ' . $zipFile . '. ZIP creation failed.'));
                    return $this->_redirectReferer();
                }
                ...
                require_once(Mage::getModuleDir('', 'Xtento_ProductExport') . DS . 'lib' . DS . 'PclZip.php');
                $zipFile = tempnam(sys_get_temp_dir(), 'zip');
            ...
            if (!$zipFile) {
                $this->_getSession()->addError(Mage::helper('xtento_productexport')->__('ZIP file couldn\'t be created.'));
                return $this->_redirectReferer();
            }
    ...
    private function _healthCheck()
    {
        ...
        if (!Mage::helper('xtento_productexport')->isModuleProperlyInstalled()) {
            if ($this->getRequest()->getActionName() !== 'installation') {
                $this->getResponse()->setRedirect($this->getUrl('*/productexport_index/installation'));
            ...
            if ($this->getRequest()->getActionName() == 'installation') {
                $this->getResponse()->setRedirect($this->getUrl('*/productexport_profile/index'));
                $this->getResponse()->sendResponse();
                $this->getRequest()->setDispatched(true);
                return $this;
            }
        ...
        // Check module status
        if (!Mage::getBlockSingleton('xtento_productexport/adminhtml_widget_menu')->isEnabled() || !Mage::helper('xtento_productexport')->getModuleEnabled()) {
            if ($this->getRequest()->getActionName() !== 'disabled') {
                $this->getResponse()->setRedirect($this->getUrl('*/productexport_index/disabled'));
                $this->getResponse()->sendResponse();
                $this->getRequest()->setDispatched(true);
                return $this;
            }
        ...
        if ($this->getRequest()->getActionName() !== 'redirect') {
            ...
            if (@!class_exists('XSLTProcessor')) {
                $this->addWarning(Mage::helper('xtento_productexport')->__('The XSLTProcessor class could not be found. This means your PHP installation is missing XSL features. You cannot export output formats using XSL Templates without the PHP XSL extension. Please get in touch with your hoster or server administrator to add XSL to your PHP configuration.'));

            }
            // Check if this module was made for the edition (CE/PE/EE) it's being run in
            if (Xtento_ProductExport_Helper_Data::EDITION !== 'EE' && Xtento_ProductExport_Helper_Data::EDITION !== '') {
                if (Mage::helper('xtcore/utils')->getIsPEorEE() && Mage::helper('xtento_productexport')->getModuleEnabled()) {
                    if (Xtento_ProductExport_Helper_Data::EDITION !== 'EE') {
                        $this->addError(Mage::helper('xtento_productexport')->__('Attention: The installed extension version is not compatible with the Enterprise Edition of Magento. The compatibility of the currently installed extension version has only been confirmed with the Community Edition of Magento. Please go to <a href="https://www.xtento.com" target="_blank">www.xtento.com</a> to purchase or download the Enterprise Edition of this extension in our store if you\'ve already purchased it.'));
                    }
                }
            }
            // Check cronjob status
            if (!Mage::getStoreConfig('productexport/general/disable_cron_warning')) {
                    ...
                    if (!Mage::helper('xtcore/utils')->isCronRunning()) {
                        if ((time() - Mage::helper('xtcore/data')->getInstallationDate()) > (60 * 30)) { // Module was not installed within the last 30 minutes
                            if (Mage::helper('xtcore/utils')->getLastCronExecution() == '') {
                                $this->addWarning(Mage::helper('xtento_productexport')->__('Cronjob status: Cron.php doesn\'t seem to be set up at all. Cron did not execute within the last 15 minutes. Please make sure to set up the cronjob as explained <a href="http://support.xtento.com/wiki/Setting_up_the_Magento_cronjob" target="_blank">here</a> and check the cron status 15 minutes after setting up the cronjob properly again.'));
                            } else {
                                $this->addWarning(Mage::helper('xtento_productexport')->__('Cronjob status: Cron.php doesn\'t seem to be set up properly. Cron did not execute within the last 15 minutes. Please make sure to set up the cronjob as explained <a href="http://support.xtento.com/wiki/Setting_up_the_Magento_cronjob" target="_blank">here</a> and check the cron status 15 minutes after setting up the cronjob properly again.'));
                            }
                        } else {
