 ...
 * Last Modified: 2013-04-28T18:10:24+02:00
 * File:          app/code/local/Xtento/ProductExport/Block/Adminhtml/Manual.php
 * Copyright:     Copyright (c) 2014 XTENTO GmbH & Co. KG <info@xtento.com> / All rights reserved.
 */

class Xtento_ProductExport_Block_Adminhtml_Manual extends Mage_Adminhtml_Block_Template
{
    public function __construct()
    {
        parent::__construct();
        $this->setTemplate('xtento/productexport/manual_export.phtml');
    }

    public function getProfileSelectorHtml()
    {
        $html = '<select class="select" name="profile_id" id="profile_id" style="width: 320px;">';
        $html .= '<option value="">' . Mage::helper('xtento_productexport')->__('--- Select Profile---') . '</option>';
        $enabledProfiles = Mage::getModel('xtento_productexport/system_config_source_export_profile')->toOptionArray();
        ...
        foreach ($profilesByGroup as $entity => $profiles) {
            $html .= '<optgroup label="' . Mage::helper('xtento_productexport')->__(ucfirst($entity) . ' Export') . '">';
            foreach ($profiles as $profile) {
                $html .= '<option value="' . $profile['value'] . '" entity="' . $entity . '">' . $profile['label'] . ' (' . Mage::helper('xtento_productexport')->__('ID: %d', $profile['value']) . ')</option>';
            }
            $html .= '</optgroup>';
        }
        $html .= '</select>';
    ...
    public function getSelectValues()
    {
        ...
        $lastEntityIds = array();
        foreach (Mage::getModel('xtento_productexport/export')->getEntities() as $entity => $label) {
            $lastEntityIds[$entity] = Mage::helper('xtento_productexport/export')->getLastEntityId($entity);
        }
        $html .= $this->arrayToJsHash('last_entity_ids', $lastEntityIds);

        $profileLinks = array();
        foreach (Mage::getModel('xtento_productexport/system_config_source_export_profile')->toOptionArray(false, false, true) as $profile) {
            $profileLinks[$profile['value']] = Mage::helper('adminhtml')->getUrl('*/productexport_profile/edit', array('id' => $profile['value']));
        }
        $html .= $this->arrayToJsHash('profile_edit_links', $profileLinks);

        $profileSettings = array();
        $settingsToFetch = array('export_filter_datefrom', 'export_filter_dateto', 'export_filter_new_only', 'start_download_manual_export');
        foreach (Mage::getModel('xtento_productexport/system_config_source_export_profile')->toOptionArray(false, false, true) as $profile) {
...
EOT;
        return $messagesBlock . $this->getLayout()->createBlock('xtento_productexport/adminhtml_widget_menu')->toHtml() . parent::_toHtml();
    }
}