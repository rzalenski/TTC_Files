 ...
 */
class Tgc_Dax_Model_Io_Sftp extends Varien_Io_Abstract implements Varien_Io_Interface
{
    ...
    public function open(array $args = array())
    {
        ...
        if (empty($args['user'])) {
            throw new Varien_Io_Exception('Username is undefined.');
        }
        if (empty($args['password'])) {
            throw new Varien_Io_Exception('Password is undefined.');
        }
        ...
        if (!$this->_connection->login($args['user'], $args['password'])) {
            throw new Varien_Io_Exception("Unable to open SFTP connection as {$args['user']}@{$args['host']}.");
        }

        if (isset($args['path'])
            && strlen($args['path'])
            && !$this->_connection->chdir($args['path'])) {

            throw new Varien_Io_Exception("Cannot change dir to {$args['path']}.");
        }
    }
