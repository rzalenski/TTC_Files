 ...
 */
class Tgc_Catalog_Helper_Product_View extends Mage_Catalog_Helper_Product_View
{
    /**
    ...
    public function initProductLayout($product, $controller)
    {
        ...
        //add handler by attribute set
        $attributeSetModel = Mage::getModel("eav/entity_attribute_set");
        $attributeSetModel->load($product->getAttributeSetId());
        $attributeSetName  = $attributeSetModel->getAttributeSetName();

        $update->addHandle('PRODUCT_ATTRIBUTE_SET_' . strtolower(str_replace(' ', '_', $attributeSetName)));

        $update->addHandle('PRODUCT_' . $product->getId());

        $goReview  = (int) $controller->getRequest()->getParam('review');
        if ($goReview) {
            $update->addHandle('product_view_go_review');
        }
    ...
    public function getSetFromProduct($product) {
        /* @var $collection Mage_Catalog_Model_Resource_Product_Link_Product_Collection */
        $collection = $product->getUpSellProductCollection();

        $collection->addAttributeToSelect('set_members');
        $collection->addAttributeToSelect('course_id');
        $collection->addAttributeToFilter('attribute_set_id', Mage::helper('tgc_catalog')->getSetAttributeSetId());
