...
class FME_Events_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	*/
	...
	public function eventsIn()
	{
		...
		$curWeek = "SELECT * FROM ".$tbl." WHERE WEEK(event_start_date) = WEEK(CURRENT_DATE)";
		
		$curMonth = "SELECT * FROM ".$tbl." WHERE MONTH(event_start_date) = MONTH(CURRENT_DATE)";
		...
		$result = $q->fetchAll();
		$daysOf = (int)Mage::getStoreConfig('events_options/basic_configs/static_block_ndaysevents');
		
		if ($daysOf > 0) {
			$result = $this->eventsByNDays($daysOf);
		}
		/* number of days given will have priority */
		return $result;
	}

	public function eventsByNDays($days = null) {
		...
		$sql = $read->select()
			->from(array('e' => $tbl));
		//$where = " DATE_ADD(NOW(),INTERVAL {$days} DAY) AND event_status = '1'";	
		
		$where = "e.event_start_date BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL {$days} DAY) AND e.event_status = 1";
		//$where = "e.event_start_date = DATE(DATE_ADD(e.event_start_date, INTERVAL 1 DAY))";
		$sql->where($where); //echo $sql->__toString();exit;
