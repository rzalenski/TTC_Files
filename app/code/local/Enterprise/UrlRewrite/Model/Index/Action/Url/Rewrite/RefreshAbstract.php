 ...
 *
 * Overridden, because we faced issues with full reindex of URL rewrites during other operations like product import,
 * product save, etc: sometimes full reindex just deleted all records in the enterprise_catalog_product_rewrite table,
 * which could appear again during import or reindex by cron process. As the result: some values were lost in
 * enterprise_catalog_product_rewrite table and at the same time records for lost products were are present in
 * enterprise_url_rewrite table.
 *
 * @category    Enterprise
    ...
    implements Enterprise_Mview_Model_Action_Interface
{
    ...
    {
        //CUSTOM CODE
        //need to wrap it into transaction, because during full reindex the import process or cron jobs may work
        //and as the result: we get url rewrites data inconsistency - some records could be lost.
        try {
