 ...
 * Overridden, because there is a Core bug with showing incorrect count of results, when 0 results were found.
 *
 * @category   Enterprise
 * @package    Enterprise_Search
...
abstract class Enterprise_Search_Model_Adapter_Abstract
{
    ...
    protected function _prepareQueryResponse($response)
    {
        ...
        if (!$_docs) {
            //CUSTOM CODE!!!
            //Bug: if found results count is 0, then setting if _lastNumFound variable was skipped,
            //and as the result the quantity of found products was taken from previous non-zero result.
            if (isset($realResponse->numFound)) {
