...
//        var stepIndex = this.steps.indexOf(this.currentStep) + 1;
        ...
        for (var i = stepIndex; i < this.steps.length; i++) {
            ...
            if ($(nextStep + '-progress-opcheckout')) {
                ...
                removeClassComplete(nextStep + '-progress-opcheckout');
                //Remove the content
    ...
    setMethod: function () {
        ...
        else if ($('no_have_pass') && $('no_have_pass').checked) {
...
Billing.prototype = {
    ...
    newAddress: function (isNew) {
        if (isNew) {
            ...
            removeValidateClass('billing-new-address-form');
            Element.show('billing-new-address-form');
            jQuery('#billing-new-address-form input[type=text]').val('');
            jQuery('#billing-new-address-form select').val('');
        } else {
    ...
    save: function () {
        ...
        if (validator.validate()) {
            ...
            $('prev_step_billing_checkout').value = checkout.prevStep;
//            if ($('billing:use_for_shipping') && $('billing:use_for_shipping').checked) {
...
//            }
    ...
    nextStep: function (transport) {
        ...
        if (checkout.prevStep == "billing" && checkout.currentStep == "review") {
...
Shipping.prototype = {
    ...
    newAddress: function (isNew) {
        if (isNew) {
            this.resetSelectedAddress();
            removeValidateClass('shipping-new-address-form');
            Element.show('shipping-new-address-form');
            jQuery('#shipping-new-address-form input[type=text]').val('');
            jQuery('#shipping-new-address-form select').val('');
        } else {
...
Review.prototype = {
    ...
    nextStep: function (transport) {
        if (transport && transport.responseText) {
            ...
            //get rid of f'ing E appended to json response
            //let me know if you find the real cause/solution for this :)
            while (text.charAt(0) === 'E') {
                text = text.substr(1);
            }
...
// Jquery customizations to checkout
function removeClassComplete(eId) {
    jQuery('#' + eId + ' dt').removeClass('complete');
}
function removeClassActive() {
    jQuery('.opcright-cont').removeClass('active');
}

jQuery(document).ready(function () {

    jQuery('.step-title').on('click', function () {
        rollBackStep = jQuery(this).attr('id').replace('step-num-', '');
        rollBackStep++;
        for (var i = rollBackStep; i <= 4; i++) {
            currentStep = '.opcright-' + i;
            jQuery(currentStep + ' dt').removeClass('complete');
            jQuery(currentStep + ' .changelink').remove();
            jQuery(currentStep + ' dd').remove();
        }
    });

});

function removeValidateClass(element){
    var input_element = '';
    jQuery('#'+element + ' div.validation-passed').each(function(){
        if (jQuery(this).find('select').length != 0){
            input_element = jQuery(this).find('select');
        }
        else if(jQuery(this).find('input').length != 0){
            input_element = jQuery(this).find('input');
        }
        if (input_element.val() == ''){
            jQuery(this).removeClass('validation-passed');
            input_element.removeClass('validation-passed');
        }
    });
    jQuery('#'+element + ' div.validation-error').each(function(){
        if (jQuery(this).find('select').length != 0){
            jQuery(this).removeClass('validation-error');
            jQuery(this).find('select').removeClass('validation-failed');
        }
        else if(jQuery(this).find('input').length != 0){
            jQuery(this).removeClass('validation-error');
            jQuery(this).find('input').removeClass('validation-failed');
        }
    });
}
